...
object ProtobufEncoderUtils {
  ...
  private def deserializerFor(typeToken: TypeToken[_], path: Option[Expression]): Expression = {
    ...
    typeToken.getRawType match {
      ...
      case other =>
        ...
        val newBuilder = StaticInvoke(other, ObjectType(builderClass), "newBuilder", returnNullable = false)
  ...
  case class InitializeProtobufBuilder(beanInstance: Expression, setters: Map[String, Expression])
    ...
    override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
      ...
      ctx.addMutableState(beanInstanceJavaType, javaBeanInstance, forceInline=true, useFreshName = false)
      ...
      val expressions = initialize.toSeq
      val initializeCode = ctx.splitExpressionsWithCurrentInputs(expressions)
      /*val initializeCode = ctx.splitExpressions(
        expressions,
        "apply",
